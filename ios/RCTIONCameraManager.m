...
@implementation RCTIONCameraManager
...
- (UIView *)view {
    return [[RCTIONCameraView alloc] initWithManager:self bridge:self.bridge];
}
...
- (NSDictionary *)constantsToExport {
    return @{
             @"Aspect": @{
                     @"stretch": @(RCTIONCameraAspectStretch),
                     @"fit": @(RCTIONCameraAspectFit),
                     @"fill": @(RCTIONCameraAspectFill)
                     },
             @"BarCodeType": @{
...
# endif
             ...
             @"Type": @{
                     @"front": @(RCTIONCameraTypeFront),
                     @"back": @(RCTIONCameraTypeBack)
                     },
             @"CaptureMode": @{
                     @"still": @(RCTIONCameraCaptureModeStill),
                     @"video": @(RCTIONCameraCaptureModeVideo)
                     },
             @"CaptureTarget": @{
                     @"memory": @(RCTIONCameraCaptureTargetMemory),
                     @"disk": @(RCTIONCameraCaptureTargetDisk),
                     @"cameraRoll": @(RCTIONCameraCaptureTargetCameraRoll)
                     },
             @"Orientation": @{
                     @"auto": @(RCTIONCameraOrientationAuto),
                     @"landscapeLeft": @(RCTIONCameraOrientationLandscapeLeft),
                     @"landscapeRight": @(RCTIONCameraOrientationLandscapeRight),
                     @"portrait": @(RCTIONCameraOrientationPortrait),
                     @"portraitUpsideDown": @(RCTIONCameraOrientationPortraitUpsideDown)
                     },
             @"FlashMode": @{
                     @"off": @(RCTIONCameraFlashModeOff),
                     @"on": @(RCTIONCameraFlashModeOn),
                     @"auto": @(RCTIONCameraFlashModeAuto)
                     },
             @"TorchMode": @{
...
- (void)reject:(RCTPromiseRejectBlock)reject
 withException:(NSException *)exception {
    NSLog(@"RCTIONCameraManager error: %@", exception);
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:[self.stillImageOutput connectionWithMediaType:AVMediaTypeVideo] completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
            if (!imageDataSampleBuffer) {
                [self reject:reject withMessage:error.description];
                return;
            }
...
- (void)saveImage:(NSData *)imageData
    ...
    if (target == RCTIONCameraCaptureTargetMemory) {
        resolve([imageData base64EncodedStringWithOptions:0]);
        return;
    }
    
    if (target == RCTIONCameraCaptureTargetCameraRoll) {
...
- (CGImageRef)newCGImageRotatedByAngle:(CGImageRef)imgRef
                                 angle:(CGFloat)angle
{
...
- (void)captureVideo:(NSInteger)target
             options:(NSDictionary *)options
            resolver:(RCTPromiseResolveBlock)resolve
            rejecter:(RCTPromiseRejectBlock)reject {
    
    if (self.movieFileOutput.recording) {
        [self reject:reject withMessage:@"Already Recording."];
        return;
    }
    
    if ([[options objectForKey:@"audio"]boolValue]) {
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        self.recordingResolve = resolve;
        self.recordingReject = reject;
...
didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL
    ...
    if (!recordSuccess) {
        [self reject:self.recordingReject withMessage:@"Error while recording."];
        return;
    }
    
    if (self.videoTarget == RCTIONCameraCaptureTargetCameraRoll) {
        ALAssetsLibrary *library = [[ALAssetsLibrary alloc] init];
        if ([library videoAtPathIsCompatibleWithSavedPhotosAlbum:outputFileURL]) {
            [library writeVideoAtPathToSavedPhotosAlbum:outputFileURL
                                        completionBlock:^(NSURL *assetURL, NSError *error) {
                                            if (error) {
                                                [self reject:self.recordingReject withMessage:error.description];
                                                return;
                                            }
                                            
                                            self.recordingResolve([assetURL absoluteString]);
                                        }];
        }
    ...
    if (self.videoTarget == RCTIONCameraCaptureTargetDisk) {
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *documentsDirectory = [paths firstObject];
        ...
        self.recordingResolve(fullPath);
        
        return;
    }
    
    [self reject:self.recordingReject withMessage:@"Target not supported."];
}
...
- (void)startSession {
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        __weak RCTIONCameraManager *weakSelf = self;
        [self setRuntimeErrorHandlingObserver:[NSNotificationCenter.defaultCenter addObserverForName:AVCaptureSessionRuntimeErrorNotification object:self.session queue:nil usingBlock:^(NSNotification *note) {
            RCTIONCameraManager *strongSelf = weakSelf;
...
RCT_EXPORT_METHOD(requestVideoPermission:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    @try {
        [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
            ...
            [self reject:reject withMessage:@"Video access is denied."];
        }];
    }
    @catch (NSException *exception) {
        [self reject:reject withException:exception];
...
RCT_EXPORT_METHOD(requestAudioPermission:(RCTPromiseResolveBlock)resolve
    ...
    @try {
        [AVCaptureDevice requestAccessForMediaType:AVMediaTypeAudio completionHandler:^(BOOL granted) {
            ...
            [self reject:reject withMessage:@"Audio access is denied."];
        }];
    }
    @catch (NSException *exception) {
        [self reject:reject withException:exception];
...
RCT_EXPORT_METHOD(capture:(NSDictionary *)options
    ...
    @try {
        ...
        if (captureMode == RCTIONCameraCaptureModeStill) {
            [self captureStill:captureTarget
                       options:options
                      resolver:resolve
                      rejecter:reject];
            return;
        ...
        if (captureMode == RCTIONCameraCaptureModeVideo) {
            [self captureVideo:captureTarget
                       options:options
                      resolver:resolve
                      rejecter:reject];
            return;
        ...
        @throw [NSException exceptionWithName:NSInvalidArgumentException
                                       reason:@"Capture mode is not valid value."
                                     userInfo:nil];
    }
    @catch (NSException *exception) {
        [self reject:reject withException:exception];
